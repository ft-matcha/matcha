...
//     try {
//         const user = await userControllers.get({ uid: req.body.uid });
//         if (user === undefined) {
...
//         const user = await userControllers.get({ uid: req.body.uid });
//         if (user === undefined) {
//             const response = await userControllers.create(req.body);
...
const logout = async (req: Request, res: Response) => {
    try {
        ...
        if (user.status === 'ACTIVE') {
            ...
            await userControllers.update(user.id, { status: 'INACTIVE' });
...
const verifyEmail = async (req: Request, res: Response) => {
    try {
        ...
        if (verify === true) {
            ...
            await userControllers.update(user.email, { verified: 1 });
            res.status(201).json({ success: true });
        } else {
