...
const LocalStrategy = new Strategy(async (uid: string, password: string, done: any) => {
    try {
        ...
        if (user === undefined) {
            return done(null, false, { message: 'Invalid uid or password' });
        } else if (user.password === crypto.createHmac('sha256', process.env.secret).update(password).digest('hex')) {
